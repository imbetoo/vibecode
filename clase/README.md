...
- **IPE entregada**: la tarjeta de IPE tiene un reloj animado y una casilla. Al marcarla, la tarjeta se atenúa, se tacha y baja por debajo de *Tareas Aula Virtual*. Se guarda en `chrome.storage.sync` (`ipeCompleted`, con la apertura de esa semana), así que se comparte entre ordenadores y se desmarca sola cuando se abre la tarea nueva (viernes 00:00).
...
| `ics-worker.js` | Web Worker que parsea el `.ics` fuera del hilo principal del popup |
